...
import { Button, Input, Select, DataTable, EmptyState, ErrorState } from '../../components/ui';
  ...
  <Card className="mb-2 shadow-sm">
    <Card.Body>
      <div className="d-flex justify-content-between">
        ...
        <Badge bg={slot.is_recurring ? 'info' : 'secondary'}>
          {slot.is_recurring ? 'Recurring' : 'Specific Date'}
        </Badge>
      </div>
      <div className="mt-2">
        <Button variant="danger" size="sm" onClick={() => onDelete(slot.id)}>Delete</Button>
      </div>
    </Card.Body>
  </Card>
);
...
const ProfessionalAvailability = () => {
  ...
  useEffect(() => {
    if (user?.isProfessional) fetchAvailability();
  }, [user]);
    ...
    <Container fluid className="px-4">
      ...
      <form onSubmit={addSlot} className="bg-light p-3 rounded mb-4">
        <Row className="g-3 align-items-end">
          ...
          <Col md={2}>
            <Input label="Start" name="start_time" type="time" value={slotForm.start_time} onChange={handleSlotChange} />
          </Col>
          <Col md={2}>
            <Input label="End" name="end_time" type="time" value={slotForm.end_time} onChange={handleSlotChange} />
          </Col>
          <Col md={2}>
            <Select
              label="Recurring?"
              ...
              options={[ { value: true, label: 'Yes (weekly)' }, { value: false, label: 'Specific date' } ]}
              onChange={handleSlotChange}
            />
          </Col>
          <Col md={2}>
            {slotForm.is_recurring === 'false' && (
              <Input label="Date" name="specific_date" type="date" value={slotForm.specific_date} onChange={handleSlotChange} />
            )}
          </Col>
          <Col md={1}>
      ...
      {availability.length === 0 ? (
        ...
        <>
          ...
          <div className="d-md-none">
            {availability.map(slot => <SlotCard key={slot.id} slot={slot} onDelete={deleteSlot} />)}
          </div>
        </>
      )}
      <Button as={Link} to="/professional" variant="outline-secondary" className="mt-3">Back to Dashboard</Button>
    </Container>
  );
};
